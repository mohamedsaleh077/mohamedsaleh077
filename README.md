## Hi there! 👋

**I'm Mohamed Saleh** — Student at the Faculty of Science and a Full Stack Web Developer (PHP & MySQL). 

A builder of web apps and simple, maintainable backend systems. Always learning, always shipping. 🚀

## About Me

## About Me

- 🎓 Student at the Faculty of Science
- 💻 Full Stack Web Developer — focused on PHP & Databases
- 🔎 Passionate about clean backend architecture, lightweight web apps and small, practical tools
- 📬 Open to opportunities — hireable
- 🗓 GitHub since: **2025-05-12**

**My Personal Website**: https://mohamedsaleh077.github.io/


## Skills & Technologies
[![My Skills](https://skillicons.dev/icons?i=php,mysql,html,css,git,github,linux,bash,phpstorm,md,js,tailwind,bootstrap,jquery,cpp,docker,windows,py,arch,bsd,bun,npm,debian,discord,gmail,nix,stackoverflow,ai,vscodium,postgres,postman,linkedin,octave,matlab,mint,selenium,ubuntu,vim&perline=8)](https://skillicons.dev)

## Top Projects

Here are a few highlights from my repositories — a mix of web apps, demos and personal projects.

1. [MyHomeLap](https://github.com/mohamedsaleh077/MyHomeLap) ⭐️ **6 stars**
   - Description: (Repo demo / project) — see README for usage and screenshots.
   - Primary language: (mixed) — check repository for details
   - Watchers: 6 • Forks: 0 • Open issues: 0

2. [TONT-PROJECT](https://github.com/mohamedsaleh077/TONT-PROJECT) ⭐️ **3 stars**
   - Description: Demo for a school management system and Education Community. Presented by Code-Hatsu — won fourth place in Egypt in the Digitopia 2025 competition (Ministry of Communications & IT).
   - Primary language: PHP
   - Watchers: 3 • Forks: 0 • License: BSD 3-Clause

3. [multaqa.live](https://github.com/mohamedsaleh077/multaqa.live) ⭐️ **2 stars**
   - Description: Live/dynamic version of the Multaqa project.
   - Primary language: PHP
   - Watchers: 2 • Forks: 3

4. [santaroso-og-v3.1](https://github.com/mohamedsaleh077/santaroso-og-v3.1) 
   - Description: (Fork/upgrade) lightweight anonymous image board — updated v3.1.
   - Primary language: PHP

5. [mohamedsaleh077.github.io](https://github.com/mohamedsaleh077/mohamedsaleh077.github.io) ⭐️ **1 star**
   - Description: My online profile / GitHub Pages site.
   - Primary language: HTML
   - GitHub Pages enabled • Watchers: 1

6. [BAS](https://github.com/mohamedsaleh077/BAS)
   - Description: Buy And Sell — online market (PHP Amazon clone). Recently created and actively developed.
   - Primary language: (PHP / PHP stack)

For details and demos, open the project links above. Each repository contains a README with setup and usage instructions. 🚀
